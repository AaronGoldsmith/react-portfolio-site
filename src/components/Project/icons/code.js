import React from "react"
const SVG =({
  width = "100%",
  height = "100%",
  viewBox = "-50 -90 250.00001 450.00001"
}) => (
<svg width={width} height={height} version="1.1" viewBox={viewBox} xmlns="http://www.w3.org/2000/svg">
 <g transform="rotate(35) translate(-100 -652.36)" fill="none" stroke="#555" strokeLinecap="round" strokeLinejoin="round" strokeWidth="38.966">
  <path d="m125.28 600.91-101.56 101.56 101.56 101.56"/>
  <path d="m376.97 600.91 101.56 101.56-101.56 101.56"/>
  <path d="m274.86 570.23-54.621 265.77"/>
 </g>
</svg>)
export default SVG